      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12 perspective-[2000px]">
          {services.map((service, index) => (
            <div
              key={service.id}
              className={`curvy-card flex flex-col group bg-white/[0.02] backdrop-blur-md border border-white/5 p-8 md:p-10 rounded-[2.5rem] hover:bg-white/[0.05] hover:border-white/20 transition-all duration-700 relative overflow-hidden transform-gpu shadow-[0_0_0_rgba(0,0,0,0)] hover:shadow-[0_20px_40px_rgba(0,0,0,0.4)] ${
                index % 2 !== 0 ? "md:mt-32" : ""
              }`}
            >
